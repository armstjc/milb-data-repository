...
def get_milb_player_team_season_stats(
    season: int, level: str, team_id: int, stats_type="batting"
):
    """ """
    now = datetime.now()
    row_df = pd.DataFrame()
    ...
    if stats_type.lower() == "batting":
        ...
        for i in json_data["stats"]:
            ...
            if i["positionAbbrev"] == i["primaryPositionAbbrev"]:
                ...
                row_df["player_position"] = (
                    i["primaryPositionAbbrev"] + "/" + i["positionAbbrev"]
                )
        ...
        game_df.loc[game_df["batting_AB"] > 0, "batting_AVG"] = (
            game_df["batting_H"] / game_df["batting_AB"]
        )
        ...
        game_df.loc[game_df["batting_AB"] > 0, "batting_SLG"] = (
            game_df["batting_TB"] / game_df["batting_AB"]
        )
        ...
        game_df.loc[game_df["batting_AB"] > 0, "batting_ISO"] = (
            game_df["batting_TB"] - game_df["batting_H"]
        ) / game_df["batting_AB"]
        game_df["batting_ISO"] = game_df["batting_ISO"].round(3)

        game_df.loc[game_df["batting_AB"] > 0, "batting_BABiP"] = (
            game_df["batting_H"] - game_df["batting_HR"]
        ) / (
            game_df["batting_AB"]
            - game_df["batting_SO"]
            - game_df["batting_HR"]
        ...
        game_df.loc[game_df["batting_AO"] > 0, "batting_GO/AO"] = (
            game_df["batting_GO"] / game_df["batting_AO"]
        )
        ...
        for i in json_data["stats"]:
            ...
            row_df["pitching_PI_balls"] = (
                row_df["pitching_PI"] - row_df["pitching_PI_strikes"]
            )
        ...
        game_df.loc[game_df["pitching_IP"] > 0, "pitching_RA9"] = round(
            9 * (game_df["pitching_R"] / game_df["pitching_IP"]), 3
        )
        ...
        game_df.loc[game_df["pitching_IP"] > 0, "pitching_H/9"] = round(
            9 * (game_df["pitching_H"] / game_df["pitching_IP"]), 3
        )

        game_df.loc[game_df["pitching_IP"] > 0, "pitching_HR/9"] = round(
            9 * (game_df["pitching_HR"] / game_df["pitching_IP"]), 3
        )

        game_df.loc[game_df["pitching_IP"] > 0, "pitching_BB/9"] = round(
            9 * (game_df["pitching_BB"] / game_df["pitching_IP"]), 3
        )

        game_df.loc[game_df["pitching_IP"] > 0, "pitching_SO/9"] = round(
            9 * (game_df["pitching_SO"] / game_df["pitching_IP"]), 3
        )

        game_df.loc[game_df["pitching_AB"] > 0, "pitching_BABiP"] = round(
            (game_df["pitching_H"] - game_df["pitching_HR"])
            / (
                game_df["pitching_AB"]
                - game_df["pitching_SO"]
                - game_df["pitching_HR"]
        ...
        game_df.loc[game_df["pitching_BB"] > 0, "pitching_SO/BB"] = round(
            game_df["pitching_SO"] / game_df["pitching_BB"], 3
        )

        game_df.loc[game_df["pitching_AB"] > 0, "pitching_BA"] = round(
            game_df["pitching_H"] / game_df["pitching_AB"], 3
        )
        ...
        game_df.loc[game_df["pitching_AB"] > 0, "pitching_SLG"] = round(
            (
                game_df["pitching_H"]
                + (game_df["pitching_2B"] * 2)
                + (game_df["pitching_3B"] * 3)
                + (game_df["pitching_HR"] * 4)
            )
            / game_df["pitching_AB"],
            3,
        )

        game_df.loc["pitching_OPS"] = round(
            game_df["pitching_OBP"] + game_df["pitching_SLG"], 3
        )

        game_df.loc["pitching_ISO"] = round(
            game_df["pitching_SLG"] - game_df["pitching_BA"], 3
        )

        game_df.loc[game_df["pitching_BF"] > 0, "pitching_SO%"] = round(
            game_df["pitching_SO"] / game_df["pitching_BF"], 3
        )

        game_df.loc[game_df["pitching_SO"] > 0, "pitching_BB/SO"] = round(
            game_df["pitching_BB"] / game_df["pitching_SO"], 3
        )

        game_df.loc[game_df["pitching_BF"] > 0, "pitching_PI/PA"] = round(
            game_df["pitching_PI"] / game_df["pitching_BF"], 3
        )
...
def get_milb_player_season_stats(
    season: int, level: str, stats_type="batting", save=False
):
    """ """
    now = datetime.now()
    game_df = pd.DataFrame()
