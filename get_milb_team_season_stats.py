...
def get_milb_team_season_stats(
    ...
    pitching_url = f"https://bdfed.stitch.mlbinfra.com/bdfed/stats/team?stitch_env=prod&season={season}&sportId={level_id}&gameType=R&group=pitching&order=desc&sortStat=onBasePlusSlugging&stats=season&limit=200&offset=0"
    batting_url = f"https://bdfed.stitch.mlbinfra.com/bdfed/stats/team?stitch_env=prod&season={season}&sportId={level_id}&gameType=R&group=hitting&order=desc&sortStat=onBasePlusSlugging&stats=season&limit=200&offset=0"
    ...
    if stats_type.lower() == 'batting':
        ...
        #     3
        # )
        season_df.loc[season_df['batting_AB'] > 0,
                      'batting_AVG'] = round(season_df['batting_H'] / season_df['batting_AB'], 3)

        season_df.loc[season_df['batting_PA'] > 0, 'batting_OBP'] = round((season_df['batting_BB'] + season_df['batting_HBP'] + season_df['batting_SF'])/(
            season_df['batting_AB']+season_df['batting_BB'] + season_df['batting_HBP'] + season_df['batting_SF']), 3)

        season_df.loc[season_df['batting_AB'] > 0,
                      'batting_SLG'] = round(season_df['batting_TB'] / season_df['batting_AB'], 3)
        ...
        season_df.loc[season_df['batting_AB'] > 0, 'batting_ISO'] = round(
            (season_df['batting_TB'] - season_df['batting_H']) / season_df['batting_AB'], 3)

        season_df.loc[season_df['batting_AB'] > 0, 'batting_BABiP'] = round((season_df['batting_H'] - season_df['batting_HR']) / (
            season_df['batting_AB'] - season_df['batting_SO'] - season_df['batting_HR'] + season_df['batting_SF']), 3)
        ...
        season_df.loc[season_df['pitching_IP'] > 0, 'pitching_RA9'] = round(9 *
                                                                            (season_df['pitching_R'] / season_df['pitching_IP']), 3)

        season_df.loc[season_df['pitching_IP'] > 0, 'pitching_WHIP'] = round((
            season_df['pitching_BB'] + season_df['pitching_H']) / season_df['pitching_IP'], 3)

        season_df.loc[season_df['pitching_IP'] > 0, 'pitching_H/9'] = round(9 *
                                                                            (season_df['pitching_H'] / season_df['pitching_IP']), 3)

        season_df.loc[season_df['pitching_IP'] > 0, 'pitching_HR/9'] = round(9 *
                                                                             (season_df['pitching_HR'] / season_df['pitching_IP']), 3)

        season_df.loc[season_df['pitching_IP'] > 0, 'pitching_BB/9'] = round(9 *
                                                                             (season_df['pitching_BB'] / season_df['pitching_IP']), 3)

        season_df.loc[season_df['pitching_IP'] > 0, 'pitching_SO/9'] = round(9 *
                                                                             (season_df['pitching_SO'] / season_df['pitching_IP']), 3)

        season_df.loc[season_df['pitching_AB'] > 0, 'pitching_BABiP'] = round((season_df['pitching_H'] - season_df['pitching_HR']) / (
            season_df['pitching_AB'] - season_df['pitching_SO'] - season_df['pitching_HR'] - season_df['pitching_SF']), 3)

        season_df.loc[season_df['pitching_BB'] > 0,
                      'pitching_SO/BB'] = round(season_df['pitching_SO'] / season_df['pitching_BB'], 3)

        season_df.loc[season_df['pitching_AB'] > 0,
                      'pitching_BA'] = round(season_df['pitching_H'] / season_df['pitching_AB'], 3)

        season_df.loc[season_df['pitching_BF'] > 0, 'pitching_OBP'] = round((season_df['pitching_H']+season_df['pitching_BB']+season_df['pitching_HBP'])/(
            season_df['pitching_AB']+season_df['pitching_BB']+season_df['pitching_HBP']+season_df['pitching_SF']), 3)

        season_df.loc[season_df['pitching_AB'] > 0, 'pitching_SLG'] = round((season_df['pitching_H'] + (season_df['pitching_2B'] * 2) + (
            season_df['pitching_3B'] * 3) + (season_df['pitching_HR'] * 4)) / season_df['pitching_AB'], 3)

        season_df.loc['pitching_OPS'] = round(season_df['pitching_OBP'] +
                                              season_df['pitching_SLG'], 3)

        season_df.loc['pitching_ISO'] = round(season_df['pitching_SLG'] -
                                              season_df['pitching_BA'], 3)

        season_df.loc[season_df['pitching_BF'] > 0,
                      'pitching_SO%'] = round(season_df['pitching_SO']/season_df['pitching_BF'], 3)

        season_df.loc[season_df['pitching_SO'] > 0,
                      'pitching_BB/SO'] = round(season_df['pitching_BB']/season_df['pitching_SO'], 3)

        season_df.loc[season_df['pitching_BF'] > 0,
                      'pitching_PI/PA'] = round(season_df['pitching_PI']/season_df['pitching_BF'], 3)

        season_df.loc[season_df['pitching_BF'] > 0, 'pitching_HR/PA'] = round(
            season_df['pitching_HR']/season_df['pitching_BF'], 3)

        # season_df.loc[season_df['pitching_BF'] > 0, 'pitching_SO/PA'] = round(
    ...
    if save == True and stats_type == 'batting':
        season_df.to_csv(
            f'season_stats/team/{season}_{level.lower()}_season_batting_stats.csv',
            index=False
        )
    elif save == True and stats_type == 'pitching':
        season_df.to_csv(
            f'season_stats/team/{season}_{level.lower()}_season_pitching_stats.csv',
            index=False
        )
    ...
    if end_season != None:
        ...
        for s in tqdm(range(season, end_season+1)):
            try:
                ...
                print(
                    f"\nCould not get {lg_level.upper()} {stats_type} stats for the {s} season.")
    else:
        print(
